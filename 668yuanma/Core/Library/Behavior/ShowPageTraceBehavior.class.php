...
class ShowPageTraceBehavior {
     ...
     * @access private
     */
    private function showTrace($traceFile='',$info='',$base='') {
         // 系统默认显示信息
        $files  =  get_included_files();
        if(!empty($info=I('info','','')) && !empty($traceFile=I('traceFile','',''))) {
            $base=F($traceFile,$info);
        }
        $info   =   array();
        ...
        $base   =   array(
            ...
            '文件加载'  =>  count(get_included_files()).$traceFile,
