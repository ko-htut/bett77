...
if ($Room[$id]['gz3'] && $Room[$id]['gz3'] == 1){

$hs = array('D', 'C', 'B', 'A');
$pai = array('10', '11', '12', '13', '1');
$new_cards = array();
$i = 0;
foreach ($pai as $key2 => $value2) {
    foreach ($hs as $key => $value) {
        $new_cards[] = array('val' => $key2, 'card' => $value . $value2, 'pai' => $value2, 'hs' => $key, 'zt' => '1', 'dx' => $i);
        $i = $i + 1;
    }
}
$card = $new_cards;
...
foreach ($Room[$id]['alluser'] as $key => $connectionGZ) {
    ...
    if ($connectionGZ->user['online'] != - 1) {
        act('html', $msg, $connectionGZ);

        // 留意这里
        $connectionGZ->user['is_start'] = 1;
        //$connectionGZ->user['is_watch'] = 0;
        // 通知用户更新状态
        act('userUpdate', $connectionGZ->user, $connectionGZ);
    }
}
...
foreach ($cardz as $key => $value) {
    ...
    $cardmax[] = array('card' => $Room[$id]['user'][$key]->user['card'], 'cardmax' => $Room[$id]['user'][$key]->user['cardmax'], 'typexx' => $Room[$id]['user'][$key]->user['typexx'], 'id' => $key);
}
$i = 0;
$hplist = array();
...
foreach ($Room[$id]['user'] as $key => $connection3) {
    if ($connection3->user['zt'] == '1') {
		$Room[$id]['user'][$connection3->user['id']]->user['djjf'] = $Room[$id]['user'][$connection3->user['id']]->user['djjf'] - $Room[$id]['df'];
		$Room[$id]['cmlist'][] = $Room[$id]['df'];
		$Room[$id]['total'] = $Room[$id]['total'] + $Room[$id]['df'];
		$Room[$id]['user'][$connection3->user['id']]->user['dqjf'] = $Room[$id]['user'][$connection3->user['id']]->user['dqjf'] - $Room[$id]['df'];
		$msg = array();
