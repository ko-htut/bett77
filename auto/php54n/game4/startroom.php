...
foreach ($cardz as $key => $value) {
	...
	$Room[$id]['minuser'] = $key;
	$cardmax[] = array('card' => $Room[$id]['user'][$key]->user['card'], 'cardmax' => $Room[$id]['user'][$key]->user['cardmax'], 'type' => $Room[$id]['user'][$key]->user['type'], 'cbeishu' => $Room[$id]['user'][$key]->user['cbeishu'], 'id' => $key);
}
$i = 0;
$hplist = array();
...
if ($Room[$id]['type'] == '14' || $Room[$id]['type'] == '15') {
	$mp = '2';
} else {
	$mp = '0';
}
