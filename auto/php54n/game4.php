...
function reqact($data2, $connection)
{
	...
	global $bonus;
	global $extract;
	$tpl = file_get_contents("./php54n/game4/" . $data2['act'] . ".php");
...
function typexx($card, $sfbj)
{
	...
	foreach ($card as $key => $value) {
		if ($value['pai'] > 10 && $value['pai'] <= 13) {
			$gong = $gong + 1;
		}
		if ($value['pai'] == 14 && $value['hs'] == 5) {
			$xw = 1;
		}
		if ($value['pai'] == 14 && $value['hs'] == 6) {
			$dw = 1;
		}
		$cardxx[] = $value['pai'];
		$hz = $value['val'] + $hz;
	...
	if ($xw == 1 && $dw == 1) {
		$result['type'] = 16;
		$result['cbeishu'] = 10;
	} elseif ($sfbj && $count[3] == 3) {
		$result['type'] = 15;
		$result['cbeishu'] = 9;
	} elseif ($gong == 3 && max($count) == 3) {
		$result['type'] = 14;
		$result['cbeishu'] = 9;
	} elseif ($gong == 0 && max($count) == 3) {
		$result['type'] = 13;
		$result['cbeishu'] = 7;
	} elseif ($dw == 1) {
		$result['type'] = 12;
