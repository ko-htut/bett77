...
foreach ($Room[$id]['user'] as $key => $value) {
	if ($value->user['zt'] == 1) {
       ...
       if(laizi($card)){
         ...
         if($value1['pai']==0){ $dw=$key1; };
	  } 

       foreach ($cards as $key2 => $value2) {  
       $sfniu = niuniu($card);
		if (!$sfniu) {$niu = 0;} else {$niu = niu($card);}
        ...
        $lzniu[]= array('牛' =>$niu , 'card' =>$card[$dw]); 
        }
       $zuidaniu=max($lzniu) ;
       $card[$dw]['val']=$zuidaniu['card']['val'];
       $card[$dw]['pai']=$zuidaniu['card']['pai'];
       $card[$dw]['hs'] =$zuidaniu['card']['hs'];
       }
