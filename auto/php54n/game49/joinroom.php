...
/*if($rules["mode"] && $admittance['0']['0'] >= $connection->user['credits'])
{
  	$msg = array('msg' => '积分不足' . $admittance['0']['0'] . '，请联系群主上分！', 'isBackHome' => true);
...
if (!$Room[$id]) {
	...
	if (time()> $Room[$id]['xx']['endtime']+1000 && $Room[$id]['xx']['endtime']!=0) {
...
echo "----------------------------------------------------------用户列表:";
var_dump($Room[$id]['index']);
echo "-----------------------------------------------------------";

$connection->user['exitlookGame']  = false;

// foreach ($Room[$id]['user'] as $connection3) {
// 	if(!isset($connection3->user['id']))
// 	{
// 		unset($Room[$id]['user'][$connection3->user['id']]);
// 	}
// }
foreach ($Room[$id]['user'] as $connection3) {
  	$mem = $db->getOne("SELECT * FROM jz_qun WHERE open='{$roomInfo["uid"]}' AND uid='{$connection->user["id"]}'");
	...
	act('adduser', $msg, $connection,false);
	   ...
	   $msg['user']['online']=$connection->user['online'];
	   $msg['user']['zt']=$connection->user['zt'];
	   // $msg['user']['test'] = "1111111111111111111111111111111";
		 if($roomInfo["uid"] == $connection->user["id"] && $mem3['credits']) {
		   $msg['user']['credits'] = $connection->user['credits'];
	   } else if($mem['credits'])
	   {
		   $msg['user']['credits'] = $mem['credits'];
	   ...
	   act_send_lookgame('adduser',$msg,$connection3);
