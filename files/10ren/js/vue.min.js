...
function adduser(data){
    ...
    html+='<img id="banker213612" src="/app/img/bull9/bull_banker_bg.png" class="background jiurenniuniu-background" /> '
     ...
     html+='<img id="bankerAnimate1'+indexuser+'" src="/app/img/bull9/bull_banker_animate.png" class="jiurenniuniu-background2" /><div class="isReady" style="display:none"><img src="/files/d_25/images/game/ready.png " class="ready" style=""></div></div>';
     $('#member').append(html);
     if(data.user.zt==1){
...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.font = "23px 微软雅黑";
        ctx.fillStyle = "#f7d92b";
        ctx.fillText("房间号:"+sj.id, 170,295);
        ctx.fillText(time1, 375, 295);
        ctx.fillText(sj.zjs+"局", 585, 295);
       ...
       // ctx.fillText(writeContent,writeLeft,345);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(140,475-(0-(i-2)*71),521,65);
            }
            var user=sj['user'][i];
            ctx.font = "32px 微软雅黑";
            ctx.fillStyle = "#f7d92b";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,10),219, 375+(i*71));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,10), 219, 375+(i*71));
                ctx.fillText(user.dqjf, 550, 375+(i*71));
            }
            ctx.fillText( user.dqjf, 550, 375+(i*71));
        }
...
function operationButton(data){
    ...
    if(data=='2'){
        ...
        html+='              <img src="/img/bull_times_bg_blue.png"  class="operationButton-gg11"  /> '
        html+='              <div  class="operationButton-gg3"  >'
        html+='               不抢'
        html+='              </div>'
        html+='             </div> '
    ...
    if(data=='7'){
        ...
        html+='<img class="operationButton-gg" src="/img/tp.png"  /> ';
    ...
    if(isguanzhan==0)
    {
        ...
        html+='<img class="operationButton-gg11" src="/img/zb.png"  /> ';
