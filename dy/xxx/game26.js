...
function history(data) {
    ...
    for(var i = 0; i < data.length; i ++) {
        var text = "";
        if(data[i]['prizes']["big"]) {
            text = "大";
        }
        if(data[i]['prizes']["little"]) {
            text = "小";
        }
        html += '<div class="item">\n' +
            '             <div class="hjs">'+data[i]['js']+'</div>\n' +
            '             <div class="himg">\n' +
            '                 <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/t'+data[i]['result'][0]+'.png"/>\n' +
            '             </div>\n' +
            '             <div class="himg">\n' +
            '                 <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/t'+data[i]['result'][1]+'.png"/>\n' +
            '             </div>\n' +
            '             <div class="himg">\n' +
            '                 <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/t'+data[i]['result'][2]+'.png"/>\n' +
            '             </div>\n' +
...
function showtanpai(){
    ...
    mp3xx='mp3niu'+allcardxx['allniu'][index];
    mp3play(mp3xx);
}
function showothertanpai(data){
    if(data==index){
        ...
        mp3xx='mp3niu'+allcardxx['allniu'][data];
...
function adduser(data){
    ...
    var html='    <div class="member member'+indexuser+'" id="user'+data.user.id+'">'
     ...
     html+='<div style="background: rgb(0, 0, 0); z-index: 888; position: absolute; width: 100%; height: 100%; opacity: 0.5;'+onlinezt+'" class="onlinezt"></div>'
     ...
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/beastfish/zhuang.png" class="background jiurenniuniu-background1" />'
     html+='<div id="bankerAnimate'+indexuser+'" class="jiurenniuniu-background2 flash" style="border-radius:0;"></div>'
     html+='<div id="bankerAnimate1'+indexuser+'" class="jiurenniuniu-background2 flash" style="border-radius:0;"></div><div class="isReady ready" style="display:none"><img style="width: 100%;height:100%;" src="http://33cdn.bepkwl.cn/yx/img/beastfish/ready.jpg"/></div></div>';
     $('#member').append(html);
...
function jibi(data){
    ...
    for(var i=0;i<11;i++){
        var $html=$('<div class="memberCoin member'+win+lose+'"><img src="http://33cdn.bepkwl.cn/yx/img/beastfish/bull_coin.png" class="jiurenniuniu-memberCoin" /></div>');
        $('#jinbi').append($html);
...
function initroom(){
    ...
    $("#panel .item .total,#panel .item .my").css("display", "block").html("");
    $(".fly-coin").remove();
    ...
    $("img.avatar").each(function(i, item) {
        ...
        if(src.search("\/index\.php") < 0 && !window.userimg[src]) {
            ...
            image.onerror = function (ev) {
                image.src = "http://33cdn.bepkwl.cn/static/img/bull10/default_head.png";
            }
...
function xiazhu(index, fen) {
    ...
    $("#operationButton div.divCoin").find(".down").css("display", "block");
    $("#operationButton div.divCoin").find(".up").css("display", "none");

    $("#operationButton div.divCoin:eq("+index+")").find(".down").css("display", "none");
    $("#operationButton div.divCoin:eq("+index+")").find(".up").css("display", "block");
...
function yao() {
    var time=Math.ceil(new Date()/1000)-timewc;
    send('yao',{time:time});
    $("#tou-end .on").css("display", "block");
    $("#tou-end .off").css("display", "none");
}
function stopyao() {
    if($("#tou-end .on").css("display") == "none") return;
    var time=Math.ceil(new Date()/1000)-timewc;
    send('stopyao',{time:time});
    //operationButton('');
}
function showyao() {
    mp3play('shake');
...
function showxian(data){
    ...
    msgxx.img='http://33cdn.bepkwl.cn/yx/img/beastfish/c'+data.fen+'1.png';
...
function showresult(data) {
    ...
    setTimeout(function () {
        ...
        $("#panel div.open div.down1, #panel div.open div.up1").addClass("bling");

        var $wp = $("#panel .item[data-type='"+data["result"][0]+"'] .wp");
        $wp.addClass("on");
        mp3play('fish'+data["result"][0]);
    }, 1200);
    setTimeout(function () {
        $("#panel div.open div.up2").css("display", "block");
        $("#panel div.open div.down2").css("display", "block");
        $("#panel div.open div.down2, #panel div.open div.up2").addClass("bling");

        var $wp = $("#panel .item[data-type='"+data["result"][1]+"'] .wp");
        $wp.addClass("on");
        mp3play('fish'+data["result"][1]);
    }, 2200);
    setTimeout(function () {
        ...
        $("#panel div.open div.down3, #panel div.open div.up3").addClass("bling");

        var $wp = $("#panel .item[data-type='"+data["result"][2]+"'] .wp");
        $wp.addClass("on");
        mp3play('fish'+data["result"][2]);
    }, 3200);
    setTimeout(function () {
        var point = Math.ceil(data["result"][0])+Math.ceil(data["result"][1])+Math.ceil(data["result"][2]);
        mp3play('point'+point);
    }, 3900);
}
...
function qbankshowother(data){
    ...
    if(data.zt){
        if(data.type==4){
            msgxx.img='http://33cdn.bepkwl.cn/static/img/X-'+data.zt+'.png';
            showmemberTimesText2(msgxx);
            msgxx.img='http://33cdn.bepkwl.cn/static/img/bull10/bull_text_'+bankwz+'.png';
            if(bankwz != 'rob') {
        ...
        msgxx.img='http://33cdn.bepkwl.cn/static/img/bull10/bull_text_not'+bankwz+'.png';
        showmemberRobText(msgxx);
        mp3xx='buqiang';
    }
...
function overroom(data){
    ...
    var img1=new Image()
    var img2=new Image()
    img.src="/fore-end_Jok/33dz-Jox/jst/12fish.jpg";
    img1.src="/fore-end_Jok/33dz-Jox/jst/dyj.png";
    img2.src="/fore-end_Jok/33dz-Jox/jst/user-line.png";
    //img2.src="bg.png";
    var sj=data;
    img.onload = function(){
        ctx.drawImage(img,0,0,800,1297);
        //ctx.drawImage(img1,120,221,50,60);
        ...
        ctx.fillText("房间号:"+sj.id, 170,235);
        ctx.fillText(time1, 375, 235);
        ctx.fillText(sj.zjs+"局", 600, 235);

        ctx.font = "26px bolder 微软雅黑";
        ctx.fillStyle = "#5761A9";
        //var writeContent = fangzhu.skinname+'房主 ：'+fangzhu.nickname;
        //var writeLeft = (c.width-ctx.measureText(writeContent).width)/2;
        //ctx.fillText(writeContent,writeLeft,185);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#19170b";
                ctx.drawImage(img2,105,320-(0-(i-1)*70),593,65);
            }
            var user=sj['user'][i];
            ctx.font = "30px  微软雅黑";
            ctx.fillStyle = "black";
            ctx.fillText(user.id, 175, 305+(i*67));
            ctx.fillText(decode64(user.nickname).substring(0,8), 319, 305+(i*67));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }
            ctx.fillText( user.dqjf, 581, 305+(i*67));
        }
        var dataURL = c.toDataURL();
        $('#overtime').html('<div onclick="location.href=\'/portal/user/index.html\'" style="z-index: 9999;position: absolute;width: 28%;height: 6%;bottom: 3%;right: 15%;" ></div><div style="background: #000;width: 100%;height: 100%;position: absolute;z-index: 200;""></div> <img src="'+dataURL+'" style="width: 100%;height:100%;position: absolute;z-index: 9201;">')
        $('#overtime').show();
    }
}
...
function operationButton(data){
    ...
    if(data=='1'){
        ...
        html+='                        <img class="operationButton-3" src="http://33cdn.bepkwl.cn/static/img/bull10/niuniu-zhuang.png">'
        ...
        html+='                        <img class="operationButton-gg" src="http://33cdn.bepkwl.cn/static/img/bull10/niuniu-zhuang1.png"> '
        html+='                        <div class="operationButton-3-ts" >'
        html+='不上'
        html+='                        </div>'
        html+='                       </div>'
    ...
    if(data=='2'){
        html+='<div class="operationButton-3-zt" id="tou-yao" style="display: inline-block;margin: 0 2%;" onclick="yao()">'
        html+='                      <img class="operationButton-3" src="http://33cdn.bepkwl.cn/yx/img/beastfish/button2.png">'
        html+='                      <div class="operationButton-3-ts" >'
        html+='                       摇 筛'
        html+='                      </div>'
        html+='                     </div>';
        html+='                     <div class="operationButton-3-zt" id="tou-end" style="display: inline-block;margin: 0 2%;" onclick="stopyao()">'
        html+='                      <img class="operationButton-3 on" style="display: none;" src="http://33cdn.bepkwl.cn/yx/img/beastfish/button2.png">'
        html+='                      <img class="operationButton-3 off" style="display: block;" src="http://33cdn.bepkwl.cn/yx/img/beastfish/defaultbutton.png">'
        html+='                      <div class="operationButton-3-ts" >'
        html+='                       结 束'
        html+='                      </div>'
        html+='                     </div>';
    ...
    if(data=='3'){
        ...
        html+='                      </div>'
        html+='                     </div>'
        html+='                     <div class="operationButton-3-zt" id="jiurenbqz" style="display: inline-block;margin: 0 2%;" onclick="qbank(0,2)">'
        html+='                      <img class="operationButton-3" src="http://33cdn.bepkwl.cn/yx/img/beastfish/bull_button_blue.png">'
        html+='                      <div class="operationButton-3-ts" >'
        html+='                       不 抢'
        html+='                      </div>'
        html+='                     </div>';
    ...
    if(data=='4'){
        ...
        html+='              <img src="http://33cdn.bepkwl.cn/static/img/bull10/niuniu-zhuang.png"  class="operationButton-gg"  /> '
    ...
    if(data=='7'){
        html+='<div class="operationButton-3-zt" id="jiurenbqz" style="display: inline-block;" onclick="tanpaime();">';
        html+='<img class="operationButton-3" src="http://33cdn.bepkwl.cn/yx/img/beastfish/button2.png"  /> ';
        html+='<div   class="operationButton-3-ts"  style="width: 100%;">';
        html+='开 盅';
        html+='</div>';
        html+='</div>';
    }
    ...
    if(data=='8'){
        ...
        html+='<div class="operationButton-1-zt">' +
            '<img class="operationButton-1" id="zb" src="http://33cdn.bepkwl.cn/yx/img/beastfish/readyButton.png"/>' +
            '<div class="operationButton-gg1"  style="width: 100%;" onClick="zhunbei()"></div>'+
            '</div>';
    }
    ...
    if(data=='10'){
        html+='<div class="divCoin divCoin1" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(0,5);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c51.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c54.png"  class="up operationButton-gg"  />'
        html+='             </div>'
        html+='             <div class="divCoin divCoin2" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(1,10);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c101.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c104.png"  class="up operationButton-gg"  />'
        html+='             </div> '
        html+='             <div class="divCoin divCoin3" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(2,20);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c201.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c204.png"  class="up operationButton-gg"  />'
        html+='             </div>'
        html+='             <div class="divCoin divCoin4" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(3,50);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c501.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c504.png"  class="up operationButton-gg"  />'
        html+='             </div>';
    }
    // 筹码10,20,50,100
    if(data=='11'){
        html+='<div class="divCoin divCoin1" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(0,10);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c101.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c104.png"  class="up operationButton-gg"  /> '
        html+='             </div>'
        html+='             <div class="divCoin divCoin2" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(1,20);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c201.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c204.png"  class="up operationButton-gg"  /> '
        html+='             </div> '
        html+='             <div class="divCoin divCoin3" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(2,50);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c501.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c504.png"  class="up operationButton-gg"  /> '
        html+='             </div>'
        html+='             <div class="divCoin divCoin4" style="display: inline-block;'
        html+='            z-index: 200;" onclick="xiazhu(3,100);">'
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c1001.png"  class="down operationButton-gg"  /> '
        html+='              <img src="http://33cdn.bepkwl.cn/yx/img/beastfish/c1001.png"  class="up operationButton-gg"  /> '
        html+='             </div>';
    }
...
function Gold(source, target) {
    ...
    var img = new Image();
    img.src = "http://33cdn.bepkwl.cn/static/img/bull10/gold.png";
    img.onload = function() {
        move();
    }
    function move() {
...
function createRanking(data, func) {
    ...
    var width = 750;
    var height = 1216;
    var pics = ['http://33cdn.bepkwl.cn/static/img/bull10/ranking_' + game_id + '_bg.jpg', '/themes/game/Public/zhongyi/img/userbg.png', '/themes/game/Public/zhongyi/img/winboss.png'];
    if (users.length > 16) {
        pics.push('http://33cdn.bepkwl.cn/static/img/bull10/people_bg2.jpg');
        pics.push('http://33cdn.bepkwl.cn/static/img/bull10/people_bg3.jpg');
    ...
    for (var i in pics) {
        if(!window.userimg[pics[i]]) {
            ...
            img.onerror = function () {
                this.src = "http://33cdn.bepkwl.cn/static/img/bull10/default_head.png";
            };
            img.src = pics[i];
        } else {
