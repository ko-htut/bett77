...
function adduser(data){
    ...
    var html='<div class="member member'+indexuser+'"  id="user'+data.user.id+'">'
     ...
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/sangong/player.png" class="background" id="user'+data.user.id+'"> '
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/sangong/playerWin.png" class="background" style="display: none;"> '
     html+='<div class="title">'+decode64(data.user.nickname)+'</div>'
	 html += '<div style="position: relative;"><div style="position: absolute; z-index: 880; display: none;" class="watchzt" ><img src="http://33cdn.bepkwl.cn/yx/watchzt.png"/ style="width: 100%;margin-top:1.0vh;margin-left:1.3vw "></div> <img src="' + data.user.img + '" class="avatar" /></div>',
      html+='<div class="score" data-dqjf="' + data.user.dqjf + '" data-credits="' + data.user.credits + '" data-mode="' + data.mode + '">'+credits+'</div>'
     html+='<img id="banker0 " src="http://33cdn.bepkwl.cn/yx/img/sangong/bull_banker_bg.png" class="background sangong6-banker0"> '
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/sangong/bull_banker_icon.png" class="background sangong6-background"> '
     html+='<img id="bankerAnimate'+indexuser+'" class="sangong6-bankerAnimate1" src="http://33cdn.bepkwl.cn/yx/img/bull/bull_banker_animate.png"> '
     html+='<img id="bankerAnimate1'+indexuser+'" class="sangong6-bankerAnimate1" src="http://33cdn.bepkwl.cn/yx/img/bull/bull_banker_animate.png"> '
     html+='<div class="quitBack" style="'+onlinezt+'"></div> '
     html+='<div class="isReady">'
     if(isguanzhan==0)
     {
      iszhunbeitwo = 1;  
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/sangong/readyButton.png" class="unready" '+actxx+'>' 
     }
     html+='<img src="http://33cdn.bepkwl.cn/yx/img/sangong/ready.png" class="ready" style="display: none;" ></div></div>'
     $('#member').append(html);
     if(data.user.zt==1){
...
function jibiover2(){
    ...
    $('#memberScoreText1 div').each(function(){
        ...
        var $bscore = $('#member .member'+userindex+' .score');
...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText(writeContent,writeLeft,355);

        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#054945";
                ctx.fillRect(142,581-(0-(i-2)*100),516,95);
            }
            var user=sj['user'][i];
            ctx.font = "32px 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 215, 380+(i*110));
            ctx.fillText(decode64(user.nickname).substring(0,10), 220, 450+(i*95));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
            ctx.fillText(decode64(user.nickname).substring(0,10), 220, 450+(i*95));
                ctx.fillText(user.dqjf, 581, 450+(i*95));
            }
            ctx.fillText( user.dqjf, 581, 450+(i*95));
        }
