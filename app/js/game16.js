function adduser(data){
    ...
    var html='<div class="member member'+indexuser+'" id="user'+data.user.id+'">'
     ...
     html+='<div style="position: absolute; top: 0px; left: 25%; width: 100%; line-height: 42px; text-align: center; color: rgb(96, 38, 3); font-size: 20px; display: none;">0</div></div>'
     html+='<div class="score" data-dqjf="' + data.user.dqjf + '" data-credits="' + data.user.credits + '" data-mode="' + data.mode + '">' + credits + '</div>'
     html += '<div style="position: relative;"><div style="position: absolute; z-index: 880; display: none;" class="watchzt" ><img src="/app/watchzt.png"/ style="width: 70%;margin-top:1.7vh;margin-left:0.7vw "></div></div>'
     html+='<div class="quitBack" style="'+onlinezt+'"></div>' 

      html+='<div class="isReady">'
      if(isguanzhan==0)
     {
      html+='<div class="unready" '+actxx+' style="display: none;overflow:visible;">'
      html+='<img src="http://lyuu.szlangya.cn/files/images/common/ready_button.png" style="width: 80%;" />'
      html+='<div class="innerText" style="position: absolute; top: 1px; left: 11.5px; width: 100%; height: 100%;line-height: 9vw;font-size: 4.5vw;text-align: center;color:#fff;font-weight: bold;"></div>'
      html+="</div>"   
        }
...
function startPk(data){
  ...
  var $pk1 = $('.playerPK .pk1 .score');
  var dqjf = data['user'][0]['dqjf'];
  var credits = $pk1.data("credits");
  var mode = parseInt($pk1.data("mode"));
  $pk1.html((mode == 1 ? credits : 0) + dqjf).data("dqjf", dqjf);
  ...
  var $pk2 = $('.playerPK .pk2 .score');
  dqjf = data['user'][1]['dqjf'];
  credits = $pk2.data("credits");
  mode = parseInt($pk2.data("mode"));
  $pk2.html((mode == 1 ? credits : 0) + dqjf).data("dqjf", dqjf);
  $('.playerPK').show();
  setTimeout(function(){
...
function fapai(data){
    for(var i=0;i<data.card.length;i++){
        ...
        showmycard(i),
        0<i&&function(i){
         var left=$(".myCards .card"+i).css("left");
         $(".myCards .card"+i).animate({left:0},function()
        {
         $(".myCards .card"+i).animate({left:left})})}(i)
    }
}
...
0<i&&function(i){
var left=$("#member .member"+indexuser+" .card"+i).css("left");
$("#member .member"+indexuser+" .card"+i).animate({left:0},function(){
$("#member .member"+indexuser+" .card"+i).animate({left:left})})}(i)}}
...
function scoresArea(data){//index,i
    ...
    var total=$('.totalScore .scores').html();
  	var $score = $('#member .member' + indexuser + ' .score');
    var jf = $score.html();
    $score.html(jf - data.jf);
      ...
      ctx.font = "30px bolder 微软雅黑";
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(142,473-(0-(i-2)*81.2),519,76);
            }
            var user=sj['user'][i];
            ctx.font = "31px 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(decode64(user.nickname).substring(0, 9), 225, 355+i*83);
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0, 9), 225, 355+i*83);
                ctx.fillText(user.dqjf, 540, 356-(0-i*83));
            }
            ctx.fillText(user.dqjf, 540, 356-(0-i*83));
        }
