...
class HomeController extends HomebaseController {
    ...
    public function ktguanli(){
        if($this->user['fk'] < '0'){
            $this->error('你的房卡不足50张');
            exit;
        }
      	$user = M("user")->where(["id" => $this->user["id"]])->find();
        $save['sfgl'] = '1';
        $save['fk'] = $this->user['fk'] - 0;
      	$save["credits"] = $user["credits"] + 20000;
        M('user')->where(array('id' => $this->user['id']))->save($save);
      	$data = array();
      	$data['create_time'] = date("Y-m-d H:i:s", time());
        $data['type'] = 'systerm';
        $data['notice'] = '系统操作';
      	$data["number"] = "+{$save["credits"]}";
      	$data["wallet"] = "credits";
      	$data["user_login"] = $this->user["id"];
      	M("allRecord")->add($data);
        $this->success('开通成功');
    }
    public function gbguanli(){
        if($this->user['fk'] < ''){
            $this->error('你的房卡不足50张');
            exit;
        }
  ...
  public function deleleRepet(){
    ...
    public function xiangqing(){
        ...
        $this->assign('dj',$dj);
        $this->display('xiangqing'.$dkxx['type']);
    }
   ...
   public function agentlist(){
       if($this->user['status']!=1 || strtotime($this->user['create_time'])<time()){
           $this->error('功能还未开启');
       }
       // 删除到期所属用户
       M('list')->where(['agent' => $this->user['id'], 'time' => ['LT', time()]]);
        //$map['uid'] = $this->user['id'];
       $map['agent'] = $this->user['id'];
       $qun = M('user')
           ->join('INNER JOIN `jz_list` ON `jz_user`.id=`jz_list`.uid')
           ->where($map)
           ->select();
       foreach ($qun as $v) {
            if(strtotime($v['create_time']) != $v['time']) {
                M('list')->where(['uid' => $v['uid']])->save(['time' => strtotime($v['create_time'])]);
            }
       }
       $this->assign('qun', $qun);
       $this->display();
  	...
  	// 积分操作
    public function addOrSubCredits(){
      	if(!$this->user['sfgl']){
            $this->error('邀请功能还未开启');
        }
        $uid = ceil(I('post.touid'));
        $touser = M('qun')->where(['open' => $this->user['id'], 'uid' => $uid])->find();
        $me = M('user')->where(['id' => $this->user['id']])->find();
      	$optype = I('post.opType');
      	$credits = ceil(I('post.credits'));
        if(empty($touser)) {
            echo json_encode(['status' => '0', 'info' => '用户不存在']);
            exit;
        }
      	if(0 >= $credits) {
        	exit(json_encode(['status' => 0, 'info' => (1 == $optype ? '上分' : '下分') . '失败，' . (1 == $optype ? '上分' : '下分') . '积分必须大于0！']));
        }
      	if(0 >= $me['credits'] || (1 == $optype && $credits > $me['credits'])) {
            echo json_encode(['status' => '0', 'info' => '积分不足，' . (1 == $optype ? '上分' : '下分') . '失败！']);
            exit;
        }
        if(2 == $optype && 0 >= $touser['credits']) {
            echo json_encode(['status' => '0', 'info' => '指定用户积分不足，下分失败！']);
            exit;
        }
        if(1 == $optype) {
          	M('user')->where(array('id' => $me['id']))->setDec('credits', $credits);
          	M('qun')->where(array('open' => $me['id'], 'uid' => $touser['uid']))->setInc('credits', $credits);
        } else {
          	M('user')->where(array('id' => $me['id']))->setInc('credits', $credits);
          	M('qun')->where(array('open' => $me['id'], 'uid' => $touser['uid']))->setDec('credits', $credits);
        }
      	$record = array(
        	'fromuid' => $me['id'],
          	'touid' => $touser['uid'],
          	'type' => $optype,
          	'credits' => (1 == $optype ? '+' : '-') . $credits,
          	'createtime' => time()
        );
      	M('UserOpCredits')->add($record);
        echo json_encode(['status' => '1', 'info' => (1 == $optype ? '上分' : '下分') . '成功']);
    }
    
    public function openagent(){
        $uid = ceil(I('post.uid'));
        $touser = M('user')->where(['id' => $uid])->find();
        $me = M('user')->where(['id' => $this->user['id']])->find();
        if($me['daycard'] < 1) {
            echo json_encode(['status' => '0', 'info' => '天卡不足']);
            exit;
        }
        if(empty($touser)) {
            echo json_encode(['status' => '0', 'info' => '用户不存在']);
        ...
        $agent = M('list')->where(['uid' => $uid])->find();
        if( empty( $agent ) ) {
            // 授权一天
            $time = time()+24*60*60;
            M('list')->add(['agent'=>$this->user['id'],'uid'=>$uid,'time'=>$time]);
            M('user')->where(['id'=>$uid])->save(['create_time'=>date('Y-m-d H:i:s',$time)]);
            M('user')->where(array('id' => $this->user['id']))->setDec('daycard', 1);
        }
        elseif (!empty($agent) && $agent['agent'] == $me['id']) {
            $time = $agent['time']+24*60*60;
            M('list')->where(['agent'=>$this->user['id'],'uid'=>$uid])->save(['time'=>$time]);
            M('user')->where(['id'=>$uid])->save(['create_time'=>date('Y-m-d H:i:s',$time)]);
            M('user')->where(array('id' => $this->user['id']))->setDec('daycard', 1);
        }
        echo json_encode(['status' => '1', 'info' => '授权成功']);
    }

    public function toushi(){
        if($this->user['status']!=1 || strtotime($this->user['create_time'])<time()){
            $this->error('功能还未开启');
        }
        $id=I('post.id');
        $map['agent']=$this->user['id'];
        $map['uid']=$id;
        $qun=M('list')->where($map)->find();
        if(!$qun){
            $this->error('管理权限不足');
        }
            //$save['create_time']=$this->user['create_time'];
            $save['is_grade']=I('post.zt');
            M('user')->where(array('id'=>$id))->save($save);
            $this->success('操作成功');
    }
    ...
    public function dashengkaifangchaxun() {
        $this->display();
    }
    ...
    public function gamejl(){
        ...
        $list=M('room')->where($map)->limit('20')->order('endtime desc')->select();
        if(!$list){
            $result['ok']=0;
            ...
            foreach ($list as $key => $value) {
               $html=$html.'  <div style=" width: 100%;margin-top: 1vh;">
  <div style="position: relative;"> 
  <div id="moretext" >
  <div class="gerenzx-fjh gerenzx-fjh-color">'.rommidxx($value['id']).'</div> 
    <div  class="gerenzx-jssj" onclick="location.href=\'/index.php/portal/home/xiangqing/room/'.$value['id'].'\'" >'.date("Y-m-d H:i",$value['time']).'</div> 
    <div class="gerenzx-dqjf gerenzx-fjh gerenzx-fjh-color" >已完成</div>
  </div>
  </div>
  </div>';
            }
